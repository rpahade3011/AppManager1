...
import com.appman.appmanager.ui.adapters.SystemAppsAdapter;
import com.appman.appmanager.util.AppUtil;
import com.yalantis.phoenix.PullToRefreshView;

public class FragmentSystemApp extends BaseFragment {
    private static final String TAG = "FragmentSystemApp";

    private PullToRefreshView pullToRefreshView;
    private RecyclerView recyclerView;

    private SystemAppsAdapter adapter;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater,
                             @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View userAppView = inflater.inflate(R.layout.layout_fragment_system_app, container, false);
    ...
    protected void setAdapter() {
        ...
        if (AppUtil.getInstance().getSystemAppsList() != null
                && !AppUtil.getInstance().getSystemAppsList().isEmpty()) {
            showErrorLayout(false);
            adapter = new SystemAppsAdapter(AppUtil.getInstance().getSystemAppsList(), activityContext);
    ...
    public void setPullToRefreshView(final PullToRefreshView pullToRefreshView) {
        pullToRefreshView.setOnRefreshListener(() -> {
            ...
            pullToRefreshView.postDelayed(() -> pullToRefreshView.setRefreshing(false),
                    2000);
        });
    }
        ...
        if (isVisible() && getActivity() != null) {
            switch (intent.getAction()) {
                ...
                case AppConstants.ACTION_PERFORM_SEARCH_CALLBACK:
                    ...
                    if (query.isEmpty()) {
                        ((SystemAppsAdapter) recyclerView.getAdapter()).getFilter().filter("");
                        noResults.setVisibility(View.GONE);
                    } else {
                        ((SystemAppsAdapter) recyclerView.getAdapter()).getFilter().filter(query.toLowerCase());
                    }
                    break;
            }
        }
